...
#define ACTION_RADIUS 350 // how far from "killed" event place player is counted as participant of this SideMission

hint localize format["+++ eventKilledAtSM.sqf: %1, veh %2, killer %3", _this, typeOf (_this select 0), name (_this select 1)];
...
_plist    = []; // list of found participaiting players
...
//
_add_to_list = {
    if (typeName _this != "STRING") exitWith { hint localize format["---SYG_eventKilledAtSM: error in call to _add_to_list ()", _this] }; // only string accepted as argument
    if (!(_this in _plist)) then {_plist set [count _plist, _this]};
};

// 1. count killer and all players in his vehicle
if ( !(isNull _killer) ) then {
    if (_killer != _unit) then { // killer is not itself
        {

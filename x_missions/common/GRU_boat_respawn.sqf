...
_marker = [GRU_BOAT_MARKER, _marker_pos, "ICON", "ColorRed",[0.7,0.7], localize "STR_GRU_BOAT_NAME", 0, _marker_type] call XfCreateMarkerLocal;
...
while { true } do {
	...
	if ( !alive _veh ) then {
		...
		if (!isNull _vah) then {
			...
			_veh = createVehicle [_type, _pos, [], 1, "NONE"];
			_veh say "return";
			_marker setMarkerType _marker_type; // remove marker
			// TODO: send msg about new GRU boat arrived STR_GRU_BOAT_ARRIVED "SM continues. A new GRU boat has appeared at the starting point. Probably with the direct help of the islanders."
		};
	};
	if (_marker != "") then {

...
_set_detected = {
	...
	if (isNil "_detected") exitWith {
		...
		if (_veh isKindOf "Truck") then {
			...
			["remote_execute", "d_radar_truck setVariable[""DETECTED"", true];", name player] call XSendNetStartScriptServer;
		} else {
		 	// ["log2server", name player,format["radar mast detected at %1", _veh call SYG_nearestLocationName]];
			["remote_execute", "d_radar setVariable[""DETECTED"", true];", name player] call XSendNetStartScriptServer;
		};
		true
	};
	false
...
if (_send_was_at_sm) then {
	_plist = [RADAR_ZONE_CENTER, INSTALL_RADIUS] call SYG_findNearestPlayers;

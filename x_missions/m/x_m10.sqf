...
if (isServer) then {
	...
	_unit = _grp createUnit [_crewman, (_arti_pos_dir select 0), [], 0, "NONE"];[_unit] join _grp;_unit setSkill 1;_unit assignAsGunner _vehicle;_unit moveInGunner _vehicle;
	extra_mission_remover_array = extra_mission_remover_array + _unit;
	_vehicle lock true;
	sleep 2.21;
	["specops", 1, "basic", 1, _poss,0] spawn XCreateInf;
	sleep 2.25;
	["specops", 0, "basic", 1, x_sm_pos select 2,0] spawn XCreateInf; // one more guarding group
	sleep 2.045;

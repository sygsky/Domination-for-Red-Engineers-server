/*
	...
	returns: nothing
*/
    hint localize "<<< SYG version of f_ChuteSteer called >>>";
	_Hdg = getDir _chute;
		...
		while {(player getVariable "ChuteOpen") == 1} do
		{
			...
			else
			{
				...
				switch (ACEChute_Pitch) do
				{
					...
					case "FORWARD":
					{
						if ((_Pitch) > -0.1) then // apply less brakes
						{
							_Pitch = _Pitch - 0.01;
						}
						else
			...
			if (_Fwd < 4 AND !_badLand) then {_DsdDescent = _DsdDescent - 2.25*(5 - _Fwd);};
			...
			if ((_NewDam - _OldDam > 0.0001) AND (position (vehicle player) select 2 < 10) AND !_badLand) then {_badLand = true;};

...
_nilCnt = 0; _ready_cnt = 0; _dead_cnt = 0; _cnt = 0;
{
	...
	if (!isNil "_mapped_veh") then {
		...
		if (alive _mapped_veh) then {  // if vehicle stil not found
			if (isNil "_var") then {
...
hint localize format[ "+++ make_map.sqf: DOSAAF vehicles unused %1, used %2, deleted %3, cleared %4, cnt %5", _cnt, _ready_cnt, _dead_cnt, _nilCnt, count _arr_old ];
...
if (isNull _map_marker ) then { // create base sign : "DangerEast"
	...
	_map_marker addAction[ localize "STR_BASE_TITLE_SHORT", "scripts\bonus\info.sqf", localize "STR_BASE_TITLE" ]; // "DOSAAF map: our base"
	...
	hint localize format[ "+++ scripts\bonus\make_map.sqf: Base map center (%1) found", _center_type ];
	["msg_to_user", "", [[ localize "STR_DOSAAF_MAP",  _cnt, _ready_cnt, _dead_cnt, _nilCnt]], 0, 5, false, "good_news"] call SYG_msgToUserParser;
};
...
hint localize format[ "+++ scripts\bonus\make_map.sqf: vehicles to add %1", count _arr_new];
{
	_pos = getPos _x;
	...
	hint localize format[ "+++ scripts\bonus\make_map.sqf: Map item (%1) for veh at %2 added near map marker (%3) at dist %4", typeOf _cone, _pos, getPos _cone, [getPos _cone, _map_marker] call SYG_distance2D ];
} forEach _arr_new;

_arr = DOSAAF_MAP_POS nearObjects [_cone_type, 55];
hint localize format[ "+++ scripts\bonus\make_map.sqf: %1 on map %2", _cone_type, count _arr];

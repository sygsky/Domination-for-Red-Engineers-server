...
if ( isNil "SYG_UTILS_GEO_COMPILED" ) then 	{ // generate some static information
    ...
    SYG_AirBoat_line = [[5592.84, 16576.33, 0],[20806.31,13623.12,0]]; // 1-2 points of line where usage of air vehicles and boats is allowed to get bigger bonus
...
//
// Get correct ASL position, but it isn't workig correctly(((
//
SYG_getPosASL = {
...
SYG_isNearLand = {
	...
	_ypos0 = _pos select 1;
	{
		_xpos = _xpos0 + _x;
		{
			if (!surfaceIsWater	[_xpos, _ypos0 + _x] ) exitWith {_shore = true};
		} forEach [_step,0,-_step]; // for Y
		if (_shore) exitWith {};

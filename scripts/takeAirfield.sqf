...
_cnt = 0;
{
    ...
    if ( _maxcnt > 0) then
    {
        ...
        hint localize format[ "scripts/takeAirfield.sqf: unit type list of ""%1"" %11, men %3, side %4 [%5:%6], d_base_arr %7, 1st unit side %8, group %9, pos %10, ",
...
localize format["scripts/takeAirfield.sqf: air base team (%1 men) created", _cnt];

while {!update_target} do
{
    hint localize format["scripts/takeAirfield.sqf: check_trigger list has %1 men ", count (list check_trigger) ];
    sleep 2.123;
};

hint localize format["scripts/takeAirfield.sqf: check_trigger actvated on men count %1 ", trigger_men_cnt ];
...
current_trigger setTriggerStatements["""Man"" countType thislist < d_man_count_for_target_clear", "xhandle = [-1] execVM ""x_scripts\x_target_clear.sqf"";execVM ""x_scripts\x_createnexttarget.sqf"";hint localize format[""scripts/takeAirfield.sqf: Trigger 'Man' count %1"",""Man"" countType thislist]", ""];

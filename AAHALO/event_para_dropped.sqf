...
_add_score = count _this == 3; // don't add score if count of params > 3
if (_add_score) then {
	_fmt= "";
	_sub_name = "";
} else {
...
hint localize format ["+++ event_para_dropped%1.sqf: d_ranked_a select 32 = %2, count = %3, volume %4, rad %5",_sub_name, _sc, count d_ranked_a, _volume, _radius ];
	...
	if ( (count _arr) > 0 ) then {
		...
		if ( _dist < _radius ) exitWith {
			if ( (_arr select 0) == AISPAWN ) then  {
				...
				_msgArr = [ "msg_to_user", "*", [[format["STR_INTRO_PARAJUMP_8%1",_fmt], _sc]], 0, 8, false, "no_more_waiting" ];
				if (_add_score) then {
				...
				_msgArr = [ "msg_to_user", "*", [[format["STR_INTRO_PARAJUMP_8_1%1", _fmt], _sc]], 0, 5, false, "losing_patience" ];
		...
		_msgArr = [ "msg_to_user", "*", [[format["STR_INTRO_PARAJUMP_8_2%1",_fmt], round(_dist), _sc]], 0, 5, false, "losing_patience" ];
		hint localize format ["+++ event_para_dropped%1.sqf: landed on dist to the one of the side circles %2 m", _sub_name, _dist];
	} else {
...
if (_send_to_server) then { // send message about circle hit to all players and print this on server
	...
	_msgArr1 = [ "msg_to_user", "*", [[format["STR_INTRO_PARAJUMP_8_ALL%1",_fmt], name player, _dist, _sc]], 0, 3, false, "no_more_waiting" ];
    ...
    if (_add_score) then {
	    ["log2server", name player, format["I hit the circle on intro! Dist. %1 m", _dist]] call XSendNetStartScriptServer;
    } else {
    	["log2server", name player, format["I hit the circle on practice! Dist. %1 m", _dist]] call XSendNetStartScriptServer;
    };

...
// Pilots rescue (sideevac) sidemission
		...
		case 56: {
			if (!alive d_radar) then {
				...
				// It is laying on the side, not staying upright
			...
			if (!alive d_radar_truck) then {
			    ...
			    // Truck is alive, sent some info about its position
...
//					_s = _s + "\n" + (format[localize "STR_RADAR_TRUCK_INFO", _name]); // "Look for the blue truck in the '%1' area"
				...
				// Truck is on the way to the target
				_pos = [d_radar_truck, 50] call SYG_MsgOnPos0;
				_s = _s + "\n" + format[localize "STR_RADAR_TRUCK_INFO2", _pos]; // "GRU truck is at %2"
				// Inform player about truck position
			};

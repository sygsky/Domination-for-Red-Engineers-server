...
SAddObserverKillScores = {
    ...
    if ( isPlayer _killer ) then {
        hint localize format["+++ SAddObserverKillScores: observer (%1) killed by %2%3 (pos %4) on dist %5",
            ...
            [_killer, 10] call SYG_MsgOnPosE0,
        ...
        if (isNull _killer) then {
            hint localize format["+++ SAddObserverKillScores: observer (%1) killed with unclear circumstances (killer is null) at %2",
                primaryWeapon _observer,
                [_observer, 10] call SYG_MsgOnPosE0
            ];
        } else {
...
x_creategroup = {
	...
	private ["_found_empty","_grp","_i","_side","_side_str","_this","_tmp_grp","_tmp_grp_a","_tmp_time"];
...
x_getwparray3 = {
	private ["_pos","_a","_b","_angle","_wp_a","_point","_i"];

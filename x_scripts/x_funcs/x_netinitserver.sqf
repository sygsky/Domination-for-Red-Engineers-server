...
*/
		...
		//
		case "remote_execute" : {
			hint localize format["+++ x_netinitserver.sqf %1", _this ];
		...
		case "base_visit_mission": {
			...
			if (typeName _equip == "STRING") then {
				_equip_arr = _equip call SYG_str2Arr;
			} else {
				_equip_arr = _equip;
			};
			 // [_wpn, _magazines, _ruck, _ruckMags, d_viewdistance, d_rebornmusic_index, base_visit_mission]

...
XfGetRanPointAnnulusBig = {
	...
	_more_call = if (count _this == 3 ) then { true } else {false}; // run additional search only if no recursion
	_center_x = _center select 0; _center_y = _center select 1;
	_ret_val = [];_co = 0;
	...
	if ( (count _ret_val == 0) && _more_call) then {
		...
		if (count _ret_val == 0) then {
			hint localize "--- XfGetRanPointAnnulusBig: 2 step not found valid pos";
...
XfGetRanPointSquare = {
//	hint localize format["+++ XfGetRanPointSquare: _this = %1", _this];
...
// Returns an average slope difference value of terrain from center to each of 8 points on circle with passed radius.
...
XfCreateMarkerLocal = {
	private ["_m_name","_m_pos","_m_shape","_m_col","_m_size","_m_text","_m_dir","_m_type","_m_brush","_marker"];
	_m_name = _this select 0;
	_m_pos = _this select 1;
	_m_shape = _this select 2;
	_m_col = _this select 3;

...
//				call compile format["_msg = localize ""STR_MAIN_COMPLETED_%1"";", _ind];
		...
		case "mt_spotted": {
            ...
            if (_soundName != "" ) then {
            	...
            	[_soundName, 20] call SYG_showMusicTitle; // It may be titled music, so try to show it
            };
		};
		...
		case "syg_observer_kill" : {
		    ...
		    if( isNull _killer ) then { // killer unknown
                _sound_obj = _this select 3; // play sound at observer position
                ...
                if ( str(_killer) == str(player) ) exitWith  { // killer is this player
                    ...
                    if (_dist < ((player call XGetRankIndexFromScore) * 10 + 10)) then { _str1 = _str1  + _str }; // #632
...
//		case "GRU_msg_patrol_detected"; // TODO: check new patrol in the future, now simply inform player about
        ...
        case "say_radio": {
        	...
        	if (count _this > 2) then { (_this select 2) call XHandleNetStartScriptClient};
        };

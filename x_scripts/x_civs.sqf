...
for "_i" from 0 to (count target_names - 1) do {
	...
	call compile format ["_trigger setTriggerStatements['thislist call x_check_for_ground', 'xhandle = [%1, thislist] execVM ''x_scripts\x_civscreate.sqf'';', 'xhandle = [%1] execVM ''x_scripts\x_civsremove.sqf'';'];", _i];
	sleep 0.501;
};
